  ...
  <div class="page-container">
    ...
    <div class="filter-bar">
      <div class="filter-left">
        <input v-model="searchForm.teacherId" type="text" placeholder="教师ID" class="filter-input" />
    ...
    <div class="table-container">
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="clazz in classList" :key="clazz.id">
            <td>{{ clazz.id }}</td>
            <td class="text-ellipsis">{{ clazz.clazzName }}</td>
            <td>{{ clazz.teacherId }}</td>
            <td class="text-ellipsis">{{ clazz.teacherName || '-' }}</td>
            <td class="action-column">
              <button class="action-btn edit" @click="openEditDialog(clazz)">编辑</button>
              <button class="action-btn delete" @click="deleteClass(clazz.id)">删除</button>
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <div class="dialog">
        ...
        <div class="dialog-body">
          <div class="form-group">
            <label>班级名称</label>
            <input v-model="formData.clazzName" type="text" placeholder="请输入班级名称" class="form-input" />
          </div>
          <div class="form-group">
            <label>教师ID</label>
...
import { clazzApi, userApi } from '../../api/api';

const dialogVisible = ref(false);
const dialogTitle = ref('添加班级');
const searchForm = ref({ teacherId: '' });
const formData = ref({ id: '', clazzName: '', teacherId: '' });
...
const loadClassList = async () => {
  try {
    ...
    const response = await clazzApi.getClazzList(params);
    if (response.code === 1) {
  ...
  } catch (error) { console.error('获取班级列表错误:', error); alert('网络错误'); }
};

const handleSearch = () => { pagination.value.pageNum = 1; loadClassList(); };
const resetSearch = () => { searchForm.value = { teacherId: '' }; pagination.value.pageNum = 1; loadClassList(); };
const handleCurrentChange = (current) => { pagination.value.pageNum = current; loadClassList(); };
const handleSizeChange = () => { pagination.value.pageNum = 1; loadClassList(); };
...
const openAddDialog = () => { formData.value = { id: '', clazzName: '', teacherId: '' }; teacherName.value = ''; teacherNameValid.value = true; dialogTitle.value = '添加班级'; dialogVisible.value = true; };
const openEditDialog = (clazz) => { formData.value = { id: clazz.id, clazzName: clazz.clazzName, teacherId: clazz.teacherId || '' }; teacherName.value = ''; teacherNameValid.value = true; dialogTitle.value = '编辑班级'; dialogVisible.value = true; };
const closeDialog = () => { dialogVisible.value = false; };

const deleteClass = async (id) => {
  if (!confirm('确定要删除这个班级吗？')) return;
  try {
    const response = await clazzApi.deleteClazz(id);
    if (response.code === 1) { alert('删除班级成功'); loadClassList(); } else { alert('删除班级失败'); }
  } catch (error) { alert('网络错误'); }
};

const submitForm = async () => {
  try {
    if (!formData.value.clazzName) { alert('班级名称为必填项'); return; }
    let response;
    if (formData.value.id) { response = await clazzApi.updateClazz(formData.value); }
    else { response = await clazzApi.addClazz(formData.value); }
    if (response.code === 1) { alert('操作成功'); closeDialog(); loadClassList(); } else { alert(response.msg || '操作失败'); }
  } catch (error) { alert('网络错误'); }
};
